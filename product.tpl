...
<div class="container main-content">
  <div class="post col-sm-8 row">
    <div class="post-head">
      <div class="product-meta">
        <div class="product-thumb col-sm-5">
          <div class="productthumbslide"> {assign var="productthumb" value=$attachdata->GetAttachByItemId($product->pid, 'product')}
    ...
    <div class="post-footer clearfix">
      <div class="pull-left"> {assign var=prevproduct value=$productdata->GetPrevProduct($product)}
        
        上一产品:
        
        {if $prevproduct->name != ""} <a href="{formaturl type="product" siteurl=$siteurl name=$prevproduct->filename}">{$prevproduct->name}</a> {else}
        
        没有了
        
        {/if} </div>
      <div class="pull-right"> {assign var=nextproduct value=$productdata->GetNextProduct($product)}
        
        下一产品:
        
        {if $nextproduct->name != ""} <a href="{formaturl type="product" siteurl=$siteurl name=$nextproduct->filename}">{$nextproduct->name}</a> {else}
